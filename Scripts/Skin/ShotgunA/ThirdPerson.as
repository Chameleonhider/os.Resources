...
namespace spades 
{
	...
	IToolSkin, IThirdPersonToolSkin, IWeaponSkin 
	{
		...
		ThirdPersonShotgunSkinA(Renderer@ r, AudioDevice@ dev) {
			@renderer = r;
			@audioDevice = dev;
			@model = renderer.RegisterModel
				("Models/Weapons/ShotgunA/Weapon3rd.kv6");
				
			@fire0 = dev.RegisterSound
				("Sounds/Weapons/ShotgunA/Fire0.wav");
			@fire1 = dev.RegisterSound
				("Sounds/Weapons/ShotgunA/Fire1.wav");
			@fire2 = dev.RegisterSound
				("Sounds/Weapons/ShotgunA/Fire2.wav");
		...
		void ReloadedWeapon() 
		{
			if(!muted)
			{
				...
				if (clientDistance < snd_maxDistance/2.f)
				{
					audioDevice.Play(pumpSound, origin, param);
				}				
			}
		}
		...
		void AddToScene() 
		{
			...
			if (readyState < 0.07f)
			{
				...
				renderer.AddSprite(flashImage, param.matrix*Vector3(0, 55, 0), 0.5f/2, 0);
			}
		}
	}
	
	IWeaponSkin@ CreateThirdPersonShotgunSkinA(Renderer@ r, AudioDevice@ dev) {
		return ThirdPersonShotgunSkinA(r, dev);
	}
}
