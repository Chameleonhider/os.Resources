 ...
 */
 ...
 namespace spades {
	...
	IToolSkin, IViewToolSkin, ISpadeSkin {
		...
		ViewSpadeSkinA(Renderer@ r, AudioDevice@ dev) {
			@renderer = r;
			@audioDevice = dev;
			@model = renderer.RegisterModel
				("Models/Weapons/SpadeA/Spade.kv6");
		...
		void AddToScene() {
			...
			if(actionType == spades::SpadeActionType::DigStart || actionType == spades::SpadeActionType::Dig)
	...
	ISpadeSkin@ CreateViewSpadeSkinA(Renderer@ r, AudioDevice@ dev) {
		return ViewSpadeSkinA(r, dev);
	}
}
