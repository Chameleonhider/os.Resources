 ...
 */
 ...
 namespace spades {
	...
	IToolSkin, IThirdPersonToolSkin, IWeaponSkin 
	{
		...
		ThirdPersonSMGSkinA(Renderer@ r, AudioDevice@ dev) {
			@renderer = r;
			@audioDevice = dev;
			@model = renderer.RegisterModel
				("Models/Weapons/SMGA/Weapon3rd.kv6");
				
			@fire0 = dev.RegisterSound
				("Sounds/Weapons/SMGA/Fire0.wav");
			@fire1 = dev.RegisterSound
				("Sounds/Weapons/SMGA/Fire1.wav");
			@fire2 = dev.RegisterSound
				("Sounds/Weapons/SMGA/Fire2.wav");
		...
		void AddToScene() 
		{
			...
			mat = CreateTranslateMatrix(0.35f, -0.25f, -0.2f) * mat;
	...
	IWeaponSkin@ CreateThirdPersonSMGSkinA(Renderer@ r, AudioDevice@ dev) {
		return ThirdPersonSMGSkinA(r, dev);
	}
}
