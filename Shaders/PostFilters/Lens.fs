...
void main() {
	...
	sum += val;
	val *= filtered(texture2D(texture, texCoord1.xy).xyz);
	gl_FragColor.xyz = val;
	
	val = vec3(0.0, 0.5, 1.0);
	sum += val;
	val *= filtered(texture2D(texture, texCoord1.zw).xyz);
	gl_FragColor.xyz += val;
	
	val = vec3(0.0, 1.0, 0.0);
	sum += val;
	val *= filtered(texture2D(texture, texCoord2.xy).xyz);
	gl_FragColor.xyz += val;
	
	val = vec3(1.0, 0.5, 0.0);
	sum += val;
	val *= filtered(texture2D(texture, texCoord2.zw).xyz);
	gl_FragColor.xyz += val;
	
	val = vec3(1.0, 0.4, 0.0);
	sum += val;
	val *= filtered(texture2D(texture, texCoord3.xy).xyz);
	gl_FragColor.xyz += val;
	
	val = vec3(1.0, 0.3, 1.0);
	sum += val;
	val *= filtered(texture2D(texture, texCoord3.zw).xyz);
	gl_FragColor.xyz += val;
